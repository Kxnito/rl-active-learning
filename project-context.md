...
- [ ] Final UCI dataset(s) chosen: **Proposed — Breast Cancer Wisconsin** (569 samples, binary classification) as the primary Phase 1 dev-loop dataset — small enough to iterate fast locally, clean enough that accuracy-gain-per-label signal won't be swamped by noise. **Proposed flagship dataset — Adult Census Income** (~48,842 samples, binary income classification), run on SageMaker once the environment/agent work end-to-end on Breast Cancer Wisconsin: large enough that active learning's core pitch (near-full-data accuracy from a small label fraction) is a real cost-savings number instead of a toy demo, and large enough to actually justify cloud training. Tradeoff: noisier, class-imbalanced (~76/24 split) — expect this to be slower/messier to tune than Breast Cancer Wisconsin, so it's deliberately sequenced after the dev-loop dataset, not built in parallel.
- [ ] RL algorithm chosen (DQN / PPO / bandit-first?): **Proposed — `MaskablePPO` (`sb3-contrib`)** — required, not just preferred: the unlabeled pool shrinks by one every step as labels get revealed, which plain SB3 DQN/PPO's fixed action space doesn't support. `MaskablePPO` with `env.action_masks()` handles this via action masking. Already reflected in the `data/`/`env`/`agent` scaffold and `requirements.txt`.
